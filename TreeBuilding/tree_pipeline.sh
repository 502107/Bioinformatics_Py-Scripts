#!/bin/bash
#SBATCH --mem 10G
#SBATCH -J sub_tree_pipeline
#SBATCH -c 1
#SBATCH -N 1
#SBATCH -p jic-long,nbi-long
#SBATCH -o tree_pipeline_%j.out
#SBATCH -e tree_pipeline_%j.err
...
MIN_SEQ=0
MIN_SAMPLES=0
SCRIPTS_DIR="/jic/scratch/groups/Diane-Saunders/main/misc/scripts"
...
do
    ...
    case $key in
        ...
        shift
        ;;
        *) echo "Usage: bash Tree_pipeline.sh -o OUTPUT_DIR -i consensus -n PGT -c codon3 -s 80 -l 80 -d SCRIPTS. Unkown option: $1 " >&2
        exit 1
    esac
    shift
done
...
if [ -z $OUTPUT_DIR ]
then
    echo "No output directory given! Using the default ('sorted') output directory..."
fi

if [ -z $INPUT_DIR ]
then
    echo "No input directory given! Cannot find the consensus files..."
    exit 1
fi

if [ -z $SPECIES ]
then
    echo "No name of SPECIES given! Using 'PGT' by default..."
fi

if [ -z $CODON ]
then
    echo "No codon specified! choices=codon1, codon2, codon3, codon12, codon123. Using 3rd codon by default..."
fi

if [ -z $MIN_SEQ ]
then
    echo "Threshold (0-100) for missing data in a contig not given! Using 0 by default..."
fi

if [ -z $MIN_SAMPLES ]
then
    echo "Threshold (0-100) for missing data across all samples not given! Using 0 by default..."
fi

if [ -z $SCRIPTS_DIR ]
then
    echo "No Scripts directory given. Using 'SCRIPTS' directory by default..."
fi
...
for f in $FILES
do
   python $SCRIPTS_DIR/sort_fasta.py -f $f > $OUTPUT_DIR/"$(basename $f)".sorted
done

#With all the files ordered by gene and placed in the sorted folder
#filter out which genes are relevant (have enough information)
#codon_from_fasta has multiple filtering parameters
#the most relevant are -l (minimum percentage of known bases in a sequence for acceptance)
#-s (minimum number of accepted samples percentage)

python $SCRIPTS_DIR/codon_from_fasta.py -d $OUTPUT_DIR -c $CODON -s $MIN_SEQ -l $MIN_SAMPLES
